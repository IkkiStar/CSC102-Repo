...
//class will hold the UAT space data
class InputData{
    constructor(
        seconds_data,
        latitude,
...
//This function builds the row of information to display. Sample output: <td>Legend: </td><td>value units<td>
...
//The start button is pressed and the data is updated live on the page
function start(){
...
//stop button is pressed and data no longer updates
function stop(){
    clearInterval(timer);
...
//gets and returns data to the webpage
function getData(){
...
function updateDisplay(){
    ...
    if(timeRead >=10){
        dataTable.rows["seconds_data"].innerHTML = dataRow("Time Elapsed", data[index].seconds_data, "seconds_data");
        ...
        dataTable.rows["bmpSensor_alt"].innerHTML = dataRow("BPM Altitude", data[index].bmpSensor_alt, "");
        dataTable.rows["bmpSensor_pres"].innerHTML = dataRow("BPM Pressure", data[index].bmpSensor_pres, "");
        dataTable.rows["bmpSensor_temp"].innerHTML = dataRow("BPM Temperature", data[index].bmpSensor_temp, "");
